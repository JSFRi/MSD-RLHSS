...
class TDAgent():
    ...
    def __init__(self,n_states,p_init,gamma,lam,a_i,b_i):
        ...
        self.b1,self.b2 = b_i
        # parameters of TD
        self.alpha= [0.0]*len(self.p)
        self.gamma = gamma
        self.lam  = lam
        # eligibility trace
        self.z = [0.0]*len(self.p)
    ...
    def learn(self, state, reward, state_next, phi_list):
        ...
        for i in range(len(self.z)):
            self.z[i]=self.lam*self.gamma*self.z[i]+phi_n[i]

        # update p
        for i in range(len(self.p)):
            self.p[i]=self.p[i]+self.alpha[i]*(reward+self.gamma*C_n_1-C_n)#*self.z[i]
    ...
    def c_up_c_not(self,tier,file,requestsDF):
        ...
        if fileNo in list(tier['No.']):
            if len(tier)==0:
                ...
                ### s2_not of tier(not)
                s2_not=entropy(list(tier.groupby(by='Population code').count()['Sample name']))
                ### s3_not of tier(not)
                #response_times=list(requested['response'])
                #s3_not=sum(response_times)/len(response_times)
                #s3_not=0
            ### C_not
            C_not,_=self.cost_phi([s1_not,s2_not])
            ...
            if len(tier_up)==0:
                ...
                ### s2_up of tier_up(up)
                s2_up=entropy(list(tier_up.groupby(by='Population code').count()['Sample name']))
                ### s3_up of tier_up(up)
                ...
                #s3_up=0
            ### C_up
            C_up,_=self.cost_phi([s1_up,s2_up])
