...
class env:
    ...
    def step(self,requestsDF,beta=0.05):
        ...
        ## calculate reward
        requested=requestsDF.loc[(requestsDF['request']==1) & (requestsDF['No.'].isin(self.tier['No.']))]
        xn=len(requested)
        ...
        if len(self.tier)==0:
            ...
            ### s2
            s2=entropy(list(self.tier.groupby(by='Population code').count()['Sample name']))
            ### s3
            '''
            if len(response_times)==0:
                s3=0
            else:
                s3=sum(response_times)/len(response_times)
            '''
        
        return s1,s2,rewards
    ...
    def remove_file(self,file):
        self.tier=self.tier.drop(self.tier.loc[self.tier['No.']==file['No.'].item()].index).reset_index(drop=True)
...
def hot_cold(tier,file):
    ...
    if float(file['temp'])<0.5:
        if random.random()<ratio:
            temp=1-0.5/np.exp(0.05*file['req_time'])
            file['temp']=temp
